...
const Login = ({ onLogin }) => {
    ...
    const handleLogin = async (e) => {
      e.preventDefault();
      try {
        const res = await axios.post('https://blogserver-mb2q.vercel.app/api/auth/login', { email, password });
        ...
        //Add a fallback error message for any network or server issues that may occur.
    ...
    const handleRegister = async (e) => {
      e.preventDefault();
      try {
        await axios.post('https://blogserver-mb2q.vercel.app/api/auth/register', { username, email, password });
        setIsRegister(false);
      } catch (err) {
